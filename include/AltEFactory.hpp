...
void testing_v2() {
    ...
    // // Floor 0
    // // createEntity(Position{0, 20, 0}, Shape{{4, 4, 1}}, Color{0.2, 0.8, 0.2, 1}, "Floor0", -1);

    // auto createLadder = [createEntity](const Position& basePos, int levels) {
    //     std::vector<entt::entity> rungs;
    //     for (int i = 1; i < levels; ++i) {
    //         auto rung = createEntity(
    //             Position{basePos.x + i * 1, basePos.y, basePos.z + i * 1.0f}, 
    //             Shape{{1, 1, 0}}, 
    //             Color{0.8, 0.8, 0.8, 1}, 
    //             "LadderRung" + std::to_string(i + 1), 
    //             1, 
    //             true
    //         );
    //         registry.emplace<Hoverable>(rung);
    //         registry.emplace<Interactable>(rung);
    //         registry.emplace<InteractionAction>(rung, InteractionAction{
    //             [](entt::registry& registry, entt::entity entity, std::optional<entt::entity> interactor) {
    //                 printf("Interacting with ladder rung\n");
    //                 if (!interactor.has_value()) return;
    //                 auto& interactorPos = registry.get<Position>(interactor.value());
    //                 auto& entityPos = registry.get<Position>(entity);
    //                 if (interactorPos.z <= entityPos.z) {
    //                     interactorPos.z += 1;
    //                 } else {
    //                     interactorPos.z -= 1;
    //                 }
    ...
    // for (int i = 0; i < 5; ++i) {
    //     float yOffset = 16 - i * 3;
    //     float zOffset = i * 1.0f; // Adjust z for each floor
    //     std::string floorName = "Floor" + std::to_string(i + 1);
    //     std::string wall2Name = "Wall" + std::to_string(i * 3 + 2);
    //     std::string wall3Name = "Wall" + std::to_string(i * 3 + 3);
    //     std::string wall4Name = "Wall" + std::to_string(i * 3 + 4);

    //     // Define base color for the floor
    //     float baseColorIntensity = 0.2f + i * 0.1f;
    //     // Define colors for the walls
    //     float leftRightWallColorIntensity = baseColorIntensity - 0.1f;
    //     float backWallColorIntensity = baseColorIntensity + 0.1f;

    //     // Floor
    //     createEntity(Position{0, yOffset, zOffset-0.1f}, Shape{{3, 3, 1}}, Color{baseColorIntensity, baseColorIntensity, baseColorIntensity, 1}, floorName, 0);
    //     // Ceiling
    //     if(i > 0)
    //         createEntity(Position{0, yOffset - 3, zOffset}, Shape{{3, 3, 1}}, Color{baseColorIntensity, baseColorIntensity, baseColorIntensity, 1}, "Ceiling" + std::to_string(i + 1), 0);

    //     // Additional walls for each floor
    //     createEntity(Position{-1, yOffset, zOffset}, Shape{{4, 1, 1}}, Color{backWallColorIntensity, backWallColorIntensity, backWallColorIntensity, 1}, wall2Name, 2, true); // Top wall
    //     createEntity(Position{-1, yOffset, zOffset}, Shape{{1, 3, 1}}, Color{leftRightWallColorIntensity, leftRightWallColorIntensity, leftRightWallColorIntensity, 1}, wall3Name, 2, true); // Left wall
    //     createEntity(Position{3, yOffset, zOffset}, Shape{{1, 3, 1}}, Color{leftRightWallColorIntensity, leftRightWallColorIntensity, leftRightWallColorIntensity, 1}, wall4Name, 2, true); // Right wall
    //     if(i > 0)
    //         createEntity(Position{-1, yOffset + 3, zOffset}, Shape{{4, 1, 1}}, Color{backWallColorIntensity, backWallColorIntensity, backWallColorIntensity, 1}, "BottomWall" + std::to_string(i * 3 + 5), 2, true); // Bottom wall
    // }

    // // Top floor
    // createEntity(Position{0, -4, 4}, Shape{{3, 3, 1}}, Color{0.2, 0.2, 0.8, 1}, "TopFloor", 0);


    // // Starting position for the staircase
    // Position startPos{0.0f, 16.0f, 4.0f};

    // int i = 0;
    // auto entity = createEntity(
    //     Position{startPos.x, startPos.y - i, startPos.z + static_cast<float>(i)},
    //     Shape{{2.0f, 1.0f, 0.1f}}, 
    //     Color{0.4f + i * 0.1f, 0.4f + i * 0.1f, 0.4f + i * 0.1f, 1.0f}, 
    //     "strairung", 
    //     1, 
    //     true
    // );
    // registry.emplace<Elevate>(entity, Elevate{1});

    // registry.emplace<CollisionAction>(entity, CollisionAction{
    //     [i](entt::registry& registry, entt::entity entity) {
    //         printf("Collision detected with entity %d\n", static_cast<int>(entity));
    //         auto& entityPos = registry.get<Position>(entity);
    //         auto& interactorPos = registry.get<Position>(_player); // Assuming _player is the interactor
    //         auto& elevate = registry.get<Elevate>(entity);
    //         // ?if (interactorPos.z <= entityPos.z && i != 6) {
    //         interactorPos.z += elevate.direction;
    //         // } else if (i == 0) {
    //         //     interactorPos.z += elevate.direction;
    //         // }
    //         // elevate.direction *= -1; // Toggle direction
    //     }
    // });